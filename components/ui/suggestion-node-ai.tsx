...
import {
  type SuggestionAIConfig,
  suggestionAIPlugin,
} from '@/components/editor/plugins/suggestion-ai-kit';
...
export function SuggestionLeafAI(props: PlateLeafProps<TSuggestionText>) {
  const { api, setOption } = useEditorPlugin(suggestionAIPlugin);
  const leaf = props.leaf;

  const leafId: string = api.suggestion.nodeId(leaf) ?? '';
  const activeSuggestionId = usePluginOption(suggestionAIPlugin, 'activeId');
  const hoverSuggestionId = usePluginOption(suggestionAIPlugin, 'hoverId');
...
export const SuggestionLineBreakAI: RenderNodeWrapper<SuggestionAIConfig> = ({
  api,
  element,
}) => {
  ...
  return function Component({ children }) {
    return (
      <SuggestionLineBreakAIContent suggestionData={suggestionData}>
        {children}
      </SuggestionLineBreakAIContent>
    );
  };
};

function SuggestionLineBreakAIContent({
  children,
  suggestionData,
}: {
  ...
  const { setOption } = useEditorPlugin(suggestionAIPlugin);
